...
def main():
    ...
    version = re.search(
        '^__version__\s*=\s*"(.*)"', open("CRISPResso/CRISPRessoCORE.py").read(), re.M
    ).group(1)
    ...
    setup(
        ...
        entry_points={
            "console_scripts": [
                "CRISPResso = CRISPResso.CRISPRessoCORE:main",
...
def check_installation(filename, tool_name):
    if os.path.isfile(filename):
        ...
        sys.stdout.write(
            "Sorry I cannot install %s for you, install manually and try again."
            % tool_name
        )
        return False
...
def install_dependencies():
    ...
    if (
        CURRENT_PLATFORM not in ["Linux", "Darwin"]
        and platform.architecture() != "64bit"
    ):
        sys.stdout.write(
    ...
    dependencies_already_installed = (
        flash_already_installed and needle_already_installed
    )
    ...
    if dependencies_already_installed:
        ...
        for shell_profile in shell_profiles:
            ...
            if (
                not os.path.exists(os.path.join(home, shell_profile))
                or not line_to_add in open(os.path.join(home, shell_profile)).read()
            ):

                if shell_profile is None:
                    sys.stdout.write(
                        "I cannot determine automatically the shell you are using. Please add the folder %s to your PATH manually!"
                        % BIN_FOLDER
                    )

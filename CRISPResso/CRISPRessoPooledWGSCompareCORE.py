...
def check_library(library_name):
    try:
        ...
        error(
            "You need to install %s module to use CRISPRessoPooledWGSCompare!"
            % library_name
        )
        sys.exit(1)
...
def check_output_folder(output_folder):
    ...
    if os.path.exists(quantification_summary_file):
        ...
        raise OutputFolderIncompleteException(
            "The folder %s  is not a valid CRISPRessoPooled or CRISPRessoWGS output folder."
            % output_folder
        )
...
def main():
    try:
        ...
        quantification_summary_file_1 = check_output_folder(
            args.crispresso_pooled_wgs_output_folder_1
        )
        quantification_summary_file_2 = check_output_folder(
            args.crispresso_pooled_wgs_output_folder_2
        )

        # create outputfolder and initialize the log
        get_name_from_folder = (
            lambda x: os.path.basename(os.path.abspath(x))
            .replace("CRISPRessoPooled_on_", "")
            .replace("CRISPRessoWGS_on_", "")
        )

        if not args.name:
            database_id = "%s_VS_%s" % (
                get_name_from_folder(args.crispresso_pooled_wgs_output_folder_1),
                get_name_from_folder(args.crispresso_pooled_wgs_output_folder_2),
            )
        else:
        ...
        df_comp = df_quant_1.set_index("Name").join(
            ...
            lsuffix="_%s" % args.sample_1_name,
            rsuffix="_%s" % args.sample_2_name,
        )

        df_comp["(%s-%s)_Unmodified%%" % (args.sample_1_name, args.sample_2_name)] = (
            df_comp["Unmodified%%_%s" % args.sample_1_name]
            - df_comp["Unmodified%%_%s" % args.sample_2_name]
        )
        df_comp["(%s-%s)_NHEJ%%" % (args.sample_1_name, args.sample_2_name)] = (
            df_comp["NHEJ%%_%s" % args.sample_1_name]
            - df_comp["NHEJ%%_%s" % args.sample_2_name]
        )
        df_comp["(%s-%s)_HDR%%" % (args.sample_1_name, args.sample_2_name)] = (
            df_comp["HDR%%_%s" % args.sample_1_name]
            - df_comp["HDR%%_%s" % args.sample_2_name]
        )
        df_comp[
            "(%s-%s)_Mixed_HDR-NHEJ%%" % (args.sample_1_name, args.sample_2_name)
        ] = (
            df_comp["Mixed_HDR-NHEJ%%_%s" % args.sample_1_name]
            - df_comp["Mixed_HDR-NHEJ%%_%s" % args.sample_2_name]
        )
        ...
        for idx, row in df_comp.iterrows():
            if row.isnull().any():
                warn(
                    "Skipping sample %s since it was not processed in one or both conditions"
                    % idx
                )
            else:
                crispresso_output_folder_1 = os.path.join(
                    args.crispresso_pooled_wgs_output_folder_1, "CRISPResso_on_%s" % idx
                )
                crispresso_output_folder_2 = os.path.join(
                    args.crispresso_pooled_wgs_output_folder_2, "CRISPResso_on_%s" % idx
                )
                crispresso_compare_cmd = (
                    'CRISPRessoCompare "%s" "%s" -o "%s" -n1 "%s" -n2 "%s" '
                    % (
